...
**No para corpus pequeños.** Si toda tu base de conocimiento son 3.000 tokens, ponla en el system prompt. Sin embeddings, sin vector store, sin pipeline. La infraestructura solo se gana su sitio cuando el corpus supera lo que puedes permitirte enviar cada vez.
...
## 11.2 Embeddings y búsqueda por similitud

### La intuición

Un embedding es una lista de números que representa el *significado* de un fragmento de texto. Un modelo típico produce 768, 1024 o 1536 números por texto.

La propiedad útil: **los textos con significado similar producen listas de números similares.**

«El gato se sentó en la alfombra» y «Un felino descansaba sobre el tapete» apenas comparten palabras. Sus embeddings están cerca. «El gato se sentó en la alfombra» y «Los ingresos trimestrales subieron un 12 %» comparten la palabra «los». Sus embeddings están lejos.

La búsqueda por palabras clave ve las palabras. Los embeddings ven el significado.

### Búsqueda por similitud

Guarda el embedding de cada chunk de tu base de conocimiento. Cuando llegue una pregunta, embébela y encuentra los vectores guardados más cercanos.

Esa operación es para lo que existe un **vector store**. La interfaz de NeuronAI es exactamente esta:

```php
public function similaritySearch(array $embedding, int $k = 4): iterable;
```

Le das un vector y te devuelve los `k` documentos más cercanos. `k` —cuántos chunks recuperar— se suele llamar top-K, y es una de las dos perillas que más afectan a la calidad. La otra es el tamaño de chunk (Sección 11.3).
...
Quien implemente un store propio necesita esto. Es también un buen ejemplo de un framework codificando una convención para prevenir una categoría de error.

### Tres cosas que sorprenden

**Los embeddings son específicos del modelo.** Los vectores del modelo de OpenAI no pueden compararse con los del modelo de Voyage. Son sistemas de coordenadas distintos. Cambia tu provider de embeddings y **debes reembeber todo tu corpus.** Trata el modelo de embeddings como parte de tu esquema de datos, no como un ajuste intercambiable.
...
**Las dimensiones deben coincidir con el store.** Si tu modelo de embeddings produce 1536 números y la columna de tu vector store está declarada como 1024, nada funciona. El esquema de MariaDB de los documentos escribe a fuego `VECTOR(1536)` exactamente por eso.

**Similitud no es relevancia.** Dos chunks pueden estar semánticamente cerca y solo uno de ellos responder a la pregunta. Esa es la brecha que el reranking existe para cerrar (Sección 12.7).
...
**Ollama ejecuta modelos de embedding en local, gratis.** Para todos los laboratorios de esta parte, y para muchísimos sistemas de producción, un modelo de embedding local es enteramente adecuado. Dado que la Sección 3.6 ya estableció Ollama, el RAG puede aprenderse de principio a fin a coste cero.

### Puntos clave

- Un embedding es una representación numérica del significado; los significados similares quedan cerca.
...
- Los embeddings son específicos del modelo: cambiar de modelo significa reembeberlo todo.
- Los modelos de embedding locales hacen que aprender RAG sea gratis.
...
**Presupuesto de contexto.** Recuperas chunks para ponerlos en el prompt. Un manual entero no cabe y, aunque cupiera, la aritmética de la Sección 1.4 dice que no querrías pagarlo en cada turno.
...
> Cuanto más largas sean tus unidades de texto, menos precisa será la representación por embeddings.

**Chunks pequeños** — embeddings precisos, coincidencia exacta, pero cada pieza recuperada puede carecer del contexto necesario para ser útil. Casas la frase exacta y resulta ser incomprensible sin el párrafo que la rodea.

**Chunks grandes** — contexto de sobra, pero embeddings borrosos y tokens desperdiciados. Recuperas 800 palabras para responder una pregunta que el modelo podría haber respondido con 40.

No hay un valor universalmente correcto. Hay un valor correcto *para tu contenido*, y encontrarlo es empírico.

### Los tres parámetros

**Longitud máxima.** Cuánto puede crecer un chunk. El splitter por defecto de NeuronAI usa 1.000 caracteres.

**Separador.** Dónde se permite cortar. El valor por defecto es el punto: fronteras de frase. Pero si tus documentos son Markdown con secciones encabezadas, cortar por `\n## ` produce chunks alineados con la propia estructura semántica del documento, que casi siempre es mejor que cortar por frases.
...
**Solapamiento.** Palabras arrastradas del chunk anterior al siguiente. El valor por defecto es cero.
...
> Chunk 1: «…la ventana de reembolso es de 30 días desde la entrega.»
>
> Chunk 2: «Pasado este periodo, solo hay saldo en tienda disponible.»

El chunk 2 por sí solo es irresoluble: *¿pasado qué periodo?* Con solapamiento, el chunk 2 comienza con la cola del chunk 1 y lleva su propio contexto.

Coste: texto duplicado significa más chunks, más llamadas de embedding, más almacenamiento. Un punto de partida razonable es el 10–15 % del tamaño de chunk. Cero solo es correcto cuando tus chunks son genuinamente independientes: unas preguntas frecuentes donde cada entrada se sostiene sola, un catálogo de productos.
...
| Preguntas frecuentes | Un chunk por pregunta |
...
- Chunks más largos significan embeddings más borrosos: ese es el compromiso central.
- Haz coincidir el separador con la estructura de tu contenido; no aceptes el valor por defecto.
- El solapamiento arregla los chunks que solos no significan nada; empieza en el 10–15 %.
- Elige los parámetros por evaluación, no por intuición.

## 11.4 RAG, ajuste fino, relleno de contexto y tools

### Las cuatro opciones

**1. Relleno de contexto.** Ponlo todo en el system prompt. Simple, exacto, sin infraestructura. Limitado por la context window y pagado en absolutamente cada petición.
...
**3. Tools.** Deja que el modelo consulte un sistema en vivo. Exacto, actual, estructurado.
...
| Hechos contables o consultables | Tools |
| Datos que cambian minuto a minuto | Tools |
...
Si alguien dice «queremos que el modelo conozca nuestro producto», quiere RAG. Si dice «queremos que escriba como nuestro equipo de soporte», eso es terreno de ajuste fino, y aun así un buen system prompt con tres ejemplos recorre la mayor parte del camino por una fracción del coste.

### La otra distinción que la gente confunde

**RAG para prosa. Tools para registros.**
...
- *«¿Se ha reembolsado el pedido 4471?»* → Tool. Es una fila de una tabla.
- *«¿Qué clientes tuvieron un reembolso el mes pasado?»* → Tool. Es una consulta.

Indexar tu tabla de pedidos en un vector store para responder a la segunda pregunta es un error de diseño que produce respuestas aproximadas dichas con seguridad. La búsqueda vectorial recupera cosas que *se parecen*; no computa.

### Se componen

Los mejores sistemas usan varios. La Sección 12.1 muestra que el framework lo soporta directamente: una clase `RAG` **es** un `Agent`, así que puede tener tools.

El ejemplo documentado está bien elegido: un agente de consejos de entrenamiento con una base de conocimiento de información sobre ejercicios (RAG) más una tool que lee el estado de entrenamiento actual del usuario desde la base de datos (tool calling). Prosa de la recuperación, hechos de la tool, una sola respuesta.

### Puntos clave

- Cuatro mecanismos: relleno, RAG, tools, ajuste fino.
- El ajuste fino cambia el comportamiento; el RAG suministra hechos.
- RAG para prosa, tools para registros: la confusión más común y más cara.
...
Recuperas cinco chunks, todos sobre reembolsos. Solo uno cubre la ventana de 30 días que preguntaba el usuario. Los otros cuatro son ruido, y el ruido cuesta tokens y puede distraer al modelo hacia responder desde el pasaje equivocado.

**Solución:** reranking. Recupera de forma amplia y luego repuntúa con un modelo que lea la consulta y cada documento a la vez. Sección 12.7.

### Fallo 3 — La respuesta abarca varios chunks
...
**Solución:** recuperación multiconsulta, o una K más alta más reranking. Genuinamente difícil: aquí es donde el RAG ingenuo enseña sus costuras.
...
**Solución:** esta no es una pregunta de RAG. Usa una tool, o filtrado por metadatos con búsqueda híbrida. Reconocerlo es la solución.
...
Alguien actualiza el documento de política. El vector store todavía contiene los chunks del trimestre pasado. El agente responde con seguridad desde información obsoleta.
...
El RAG ingenuo te lleva quizá el 70 % del camino. El 30 % restante es transformación de consultas, reranking, filtrado por metadatos, búsqueda híbrida y evaluación, que es exactamente por lo que el pipeline de NeuronAI tiene procesadores previos y posteriores como etapas de primera clase y no como un añadido de última hora.
...
- Seis modos de fallo: desajuste de consulta, similar pero irrelevante, respuestas repartidas entre chunks, agregación, alucinación, obsolescencia.
