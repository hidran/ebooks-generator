...
**Fíjate en que son dos modelos distintos.** Tu provider podría ser Claude; tu provider de embeddings podría ser OpenAI o un modelo local de Ollama. Son elecciones independientes, y mezclarlas es normal y no un error.
...
> La clase `RAG` de NeuronAI extiende la clase básica `Agent`. Tu RAG siempre es un agente, así que puedes engancharle tools y definir instrucciones de sistema.
...
El ejemplo de la documentación está bien elegido: consejos de entrenamiento a partir de una base de conocimiento, más una tool para el estado actual del usuario:
...
Conocimiento de la recuperación, hechos de las tools, aritmética del toolkit. Eso es el «se componen» de la Sección 11.4, en una sola clase.

::: {.callout .callout-warning}
[La firma del vector store: comprueba esto antes de escribir nada]{.callout-title}
...
- El modelo de chat y el de embeddings son elecciones independientes.
- `chat()` no cambia; la recuperación es interna.
- El RAG hereda todas las funcionalidades del Agent, incluidas las tools.

## 12.2 Data loaders y lectores
...
**Cada lector está ligado a una extensión de archivo.** El loader elige el correcto automáticamente según lo que encuentre.
...
Fíjate en que convierte el HTML **a Markdown** en lugar de quitar las etiquetas. Eso importa: los encabezados sobreviven como `##`, lo que significa que un splitter consciente de los encabezados (Sección 12.3) puede usarlos. Reducir a texto plano tiraría esa estructura a la basura.
...
**Este es el loader que más usarás en una aplicación real.** Tu base de conocimiento probablemente son filas de una tabla —artículos, descripciones de producto, registros de políticas— y no archivos en disco. El loader de archivos se lleva el espacio en la documentación; el de cadenas se lleva el uso en producción.
...
No necesitas un agente RAG para hacer la ingesta. El provider de embeddings y el vector store funcionan de forma independiente:
...
**El store debe ser el mismo que usa tu agente RAG.** Mismo directorio, mismo índice, misma colección. Este es el error de ingesta más común: indexas en un store y consultas otro, y el agente dice que no sabe nada.

**¿Por qué usar componentes autónomos?** Porque la ingesta y la consulta son trabajos distintos con ciclos de vida distintos. La ingesta es un proceso por lotes, un cron, un worker de cola. No necesita un provider de chat configurado, ni una clave de API para el LLM, ni las instrucciones del agente. Separarlos mantiene tu script de ingesta ligero y lo hace desplegable de forma independiente.
...
- El HTML se convierte a Markdown, preservando la estructura para el splitter.
- `StringDataLoader` es lo que usarás de verdad: la mayoría de las bases de conocimiento son filas de base de datos.
- Haz la ingesta con componentes autónomos; el store debe ser idéntico al del agente.

## 12.3 Splitters

### Enganchar un splitter
...
- **`maxLength`** — chunks no más largos que esto
- **`separator`** — dónde se permite cortar; el punto por defecto
- **`wordOverlap`** — palabras arrastradas entre chunks; cero por defecto
...
Divide en frases, las agrupa en chunks basados en número de palabras y opcionalmente solapa por palabras.
...
### Splitters propios: el código con más palanca de tu RAG
...
Dos métodos. Aquí va un splitter por encabezados de Markdown —quizá cuarenta líneas— que superará a cualquier splitter genérico sobre documentación:
...
Cada chunk que esto produce es una sección completa y autocontenida con su propio encabezado. Un acierto de recuperación trae de vuelta una unidad coherente en lugar de una ventana arbitraria de 1.000 caracteres que empieza a mitad de frase.
...
Es además una elección legítima de producción. Los modelos de embedding son pequeños y rápidos; ejecutar uno en local es muy distinto de ejecutar en local un modelo de chat de frontera.

### Opciones alojadas

`OpenAIEmbeddingsProvider`, `VoyageEmbeddingsProvider` y otros. Voyage en particular merece conocerse: se especializa en embeddings para recuperación y a menudo supera a los modelos de propósito general en benchmarks de RAG.
...
La Sección 3.6 enseñó que cambiar de provider es un cambio de una línea. **Los embeddings son la excepción.**

Cambia el modelo de embeddings y todos los vectores de tu store se vuelven carentes de sentido. Son coordenadas en otro espacio. La recuperación devolverá resultados —no fallará ruidosamente— y estarán mal.

**Cambiar el modelo de embeddings significa reembeber todo el corpus.**

Tres consecuencias prácticas:

- Registra qué modelo produjo tu índice. En el nombre del store, en un campo de metadatos, en una nota de despliegue: en algún sitio.
- Presupuesta el tiempo de reembebido antes de cambiar. Millones de chunks son horas y dinero real.
- Nunca hagas del modelo de embeddings una variable de entorno que difiera entre entornos. Desarrollo con `nomic-embed-text` y producción con `text-embedding-3-large` significa que tu índice de desarrollo y el de producción son incompatibles, y el error será desconcertante.
...
Distintos modelos producen vectores de distinta longitud: 768, 1024, 1536 y otras. Tu vector store debe estar configurado para coincidir. `TypesenseVectorStore` recibe `vectorDimension: 1024`; el esquema de MariaDB declara `VECTOR(1536)`.

Un desajuste significa o un error duro o disparates silenciosos, según el store. Compruébalo al configurarlo.
...
- **Cambiar el modelo de embeddings invalida todo tu índice.**
- Nunca varíes el modelo de embeddings entre entornos.
- Las dimensiones de los vectores deben coincidir con la configuración de tu store.
...
**Memory** — volátil, solo la sesión actual. Para tests e interacciones desechables.
...
::: {.callout .callout-warning}
[No hay store pgvector]{.callout-title}

La lista de arriba está completa. Gran cantidad de material de terceros asume que NeuronAI incluye una integración con pgvector, porque pgvector es omnipresente en el ecosistema Python. No la incluye. Si estás en Postgres y quieres soporte de primera parte, tus opciones son PHPVector (al que le da igual qué base de datos ejecutes) o uno de los stores dedicados.
:::
...
Varios stores admiten filtrar por metadatos junto a la similitud vectorial. El patrón que da la documentación:
...
El ejemplo de Pinecone llama a `withFilters()` y el de Elasticsearch a `withFilter()`. Comprueba ambos contra tu store. Apéndice A, punto 26.
:::
...
Los mantenedores invitan explícitamente a enviar pull requests con nuevos stores. Si quieres una contribución de código abierto bien acotada y genuinamente útil, esta es una.
...
Campos propios guardados junto a los campos por defecto del documento en el store.
...
**Tenant o propietario.** El límite de seguridad para la recuperación multi-tenant. No negociable si sirves a más de un cliente desde un índice.
...
Una vez los campos están en el store, las bases de datos compatibles pueden restringir una búsqueda semántica a los registros que cumplen criterios sobre otros campos, en lugar de comparar solo los embeddings vectoriales.
...
Si `sourceType` y `sourceName` ya existen en el store, **esos documentos se borran y se sustituyen** por los chunks de la nueva versión. Todo lo demás se guarda normalmente.

Esa es la operación que resuelve el modo de fallo 6 de la Sección 11.5, y es por lo que `deleteBySource()` está en la interfaz del store.
...
Renombra el archivo, reindexa y tendrás ambas versiones en el store: los chunks viejos huérfanos y no borrables por fuente, y los nuevos junto a ellos. El agente recuperará de ambos y responderá desde el que haya casado mejor.
...
- Adjunta tenant, visibilidad, idioma y frescura por defecto.
- Filtra los permisos **en la recuperación**: el postfiltrado es una fuga.
- `reindexBySource()` sustituye los chunks de una fuente; `sourceName` debe ser estable.
- Usa IDs de registro, no títulos, como nombres de fuente.

## 12.7 El workflow de RAG: procesadores previos y posteriores
...
UserMessage
    ...
    ├─ EnrichInstructionsNode     ← inyecta los documentos en el system prompt
    ├─ ChatNode                   ← ejecuta la inferencia
    └─ ToolNode                   ← ejecuta las tools, si las hay
    │
AssistantMessage
```

**Esta es la recompensa de la Sección 2.3.** Un agente RAG es un workflow, sus nodos tienen nombre, y conocer los nombres te permite enganchar el sistema con middleware. Todo lo de la Parte IV aplica aquí.
...
| La respuesta abarca varios chunks | Estrategia de recuperación + reranking |
...
Por qué ayuda: los usuarios formulan preguntas en el lenguaje de los *problemas*; los documentos están escritos en el lenguaje de las *soluciones*. «¿Por qué está roto mi trasto?» y «El código de error 4021 indica asignación de disco insuficiente» están semánticamente lejos. Un paso de transformación reescribe la pregunta en algo más cercano a cómo está formulada la respuesta —o la expande en varias variantes— antes de que ocurra el embedding.
...
`PostProcessDocumentsNode` ejecuta el pipeline de procesadores posteriores, y el reranking es la razón de que exista.

```php
use NeuronAI\RAG\PostProcessor\JinaRerankerPostProcessor;
```

**Por qué funciona el reranking, y por qué es la incorporación con mayor retorno a un sistema RAG que ya funciona:**

La búsqueda vectorial compara dos embeddings calculados *de forma independiente*. La consulta se comprimió en un vector sin saber nada de los documentos; cada documento se comprimió sin saber nada de la consulta. Se pierde información en ambas compresiones.
...
```
Búsqueda vectorial → 50 candidatos → reranking → 5 mejores → enviar al modelo
```

Obtienes la cobertura de una búsqueda amplia y la precisión de una cuidadosa, y le envías al modelo menos chunks y mejores, lo que además reduce tokens.

### Estrategia de recuperación

`RetrieveDocumentsNode` ejecuta la estrategia de recuperación, y NeuronAI permite personalizar esa estrategia, incluida la recuperación desde fuentes de datos externas y no solo desde el vector store.

Ese es el punto de extensión para cualquier cosa inusual: una búsqueda híbrida que combine BM25 y vectores, recuperación multiconsulta para preguntas que abarcan varios chunks, o tirar de una API de búsqueda junto a tu propio índice.

### EnrichInstructionsNode: donde se combate la alucinación

Este nodo añade los documentos recuperados al system prompt del agente.

Lo que significa que el system prompt es donde restringes el uso que el modelo hace de ellos. Combina el mecanismo del nodo con tus `instructions()`:
...
Como estos son nodos de workflow, el middleware los apunta por nombre: la misma API que `Neuron::middleware(ToolNode::class, ...)` de la Sección 2.3.
...
- Seis nodos: preprocesar, recuperar, posprocesar, enriquecer, chatear, tools.
- Los procesadores previos arreglan el desajuste consulta/respuesta a cambio de una llamada al modelo.
- **El reranking es la mejora con mayor retorno para un sistema RAG que ya funciona**: recupera 50, reordena, envía 5.
...
**Cubre:** clase RAG, loader de archivos, splitter propio, embeddings locales, store en archivo.
...
- La indexación reporta un número de chunks coherente con el número de encabezados `##` de tu corpus, no un número redondo que sugiera división por recuento de caracteres.
...
- Borrar el directorio del vector store y volver a ejecutar la indexación produce las mismas respuestas.
...
3. Edita un archivo fuente, vuelve a ejecutar la indexación con `reindexBySource()` y confirma que los chunks viejos han desaparecido.

## Laboratorio 9 — Store de producción con búsqueda híbrida

**Cubre:** PHPVector, metadatos, filtros, evaluación.

### Objetivo

Mover el Laboratorio 8 a un store con capacidades de recuperación reales, añadir filtrado por metadatos y medir si realmente mejoró algo.
...
Un método cambiado. Todo lo demás —el agente, el loader, el splitter, los scripts— queda intacto. Esa es la arquitectura guiada por interfaces de la Sección 2.2 dando frutos en la capa de datos, y resulta más convincente verlo ocurrir que leerlo.
...
Ejecútalo contra ambos stores y ambos splitters. Cuatro configuraciones, un número cada una.

**Esa tabla es el entregable de este laboratorio.** No el código: la medición. Es la diferencia entre «mejoramos el RAG» y «la fidelidad pasó de 0,62 a 0,81 al cambiar de splitter, y cambiar de store no cambió nada».
...
- Una consulta filtrada por metadatos demostrablemente no puede devolver documentos fuera de ese filtro; pruébalo con documentos de dos tenants en un mismo índice.

## Ejercicios del capítulo

1. **Compara splitters.** Indexa un corpus de documentación real con el splitter por defecto y con uno propio. Compara sobre quince preguntas, usando el evaluador en lugar de tu impresión.
2. **Demuestra el aislamiento.** Añade metadatos de tenant y verifica que una consulta filtrada no puede devolver documentos de otro tenant. Es un test de seguridad; escríbelo como tal.
3. **Reindexa.** Cambia un archivo fuente y reindexa con `reindexBySource()`. Confirma que los chunks viejos han desaparecido consultando una frase que borraste.
