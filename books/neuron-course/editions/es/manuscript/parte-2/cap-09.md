...
En la práctica: permite que un servidor exponga un conjunto de herramientas sobre un protocolo definido, y que cualquier cliente capaz de MCP las consuma.

### El problema que resuelve

Antes de MCP, cada integración era a medida. ¿Quieres que tu agente use Slack? Lee la documentación de la API de Slack, escribe las clases de herramienta, gestiona la autenticación, mantenlo. Luego haz lo mismo para Jira. Luego para GitHub. Luego para tu CRM. Y todos los demás frameworks en todos los demás lenguajes rehacen el mismo trabajo.
...
**El operador de expansión.** `...McpConnector::make(...)->tools()`: `tools()` devuelve un array, y la expansión lo funde en tu lista. Olvida el `...` y anidarás un array dentro del array de herramientas, cosa que falla de una forma que el error no deja clara.
...
**Lo que te cuesta:** todas y cada una de las garantías de la Sección 5.1. Esas herramientas no las escribiste tú. No las revisaste. No controlas sus descripciones, su comportamiento, su gestión de errores ni qué hacen con los argumentos que envía el modelo. La Sección 9.4 se lo toma en serio.
...
- Un estándar abierto: los servidores exponen herramientas, cualquier cliente las consume.
...
Esto requiere Nodo en la máquina que ejecuta el agente. Un desarrollador de PHP sin Nodo instalado chocará con un fallo confuso y supondrá, razonablemente, que el framework está roto. Instálalo antes de trabajar esta sección.
:::
...
En vez de construir herramientas para un agente, publicas una superficie de capacidades una sola vez. Para una empresa con un sistema interno valioso, eso es un movimiento estratégico más que un detalle de implementación.
...
- **`url`** — el punto de conexión del servidor
...
**Diferencia importante respecto a la Sección 5.8.** Los filtros de juego de herramientas reciben **nombres de clase** totalmente cualificados. Los filtros de MCP reciben **cadenas con el nombre de la herramienta**, porque las herramientas se definen remotamente y no tienen clases PHP de tu lado.

Esto tiene una consecuencia que conviene enunciar: no hay análisis estático, ni autocompletado del IDE, ni error en tiempo de compilación si un nombre cambia. Una errata en una lista de `only()` produce silenciosamente menos herramientas de las que esperabas. Registra el número resultante de herramientas durante el desarrollo.

### Usa `only()`. Siempre.

La Sección 5.8 argumentaba que las listas de permitidos ganan a las de denegación porque los juegos de herramientas ganan herramientas en las versiones del framework. Con MCP el argumento es muchísimo más fuerte:

**El servidor puede añadir herramientas en cualquier momento, sin que te enteres, sin un despliegue de tu parte.**
...
Todos los argumentos de la Sección 5.1 sobre que la lista de herramientas es tu límite de seguridad asumían que escribiste tú las herramientas. Con MCP no las escribiste.

A qué le estás extendiendo confianza:

- **Descripciones de herramientas que no escribiste.** Y las descripciones son instrucciones que el modelo lee. Una descripción maliciosa o descuidada es un vector de inyección de prompts con un mecanismo de entrega de aspecto legítimo.
