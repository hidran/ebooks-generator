...
NeuronAI tiene una capa de mensajes unificada —roles, bloques de contenido, metadatos— y es la pieza que hace que el cambio de provider funcione de verdad en lugar de solo aparentarlo.

### Por qué existe una capa de mensajes unificada

Cada provider tiene su propia forma de petición y de respuesta. OpenAI, Anthropic, Gemini y Ollama difieren en cómo representan los roles, cómo adjuntan imágenes, cómo devuelven las llamadas a tools y cómo exponen los rastros de razonamiento.

La respuesta de NeuronAI es una única abstracción de mensaje que se proyecta sobre todos ellos. Esto es lo que hace que la Sección 3.6 sea algo más que un truco de fiesta: el cambio funciona porque la capa de mensajes absorbe las diferencias. Sin ella, «cambia una línea para cambiar de provider» sería falso en el momento en que adjuntaras una imagen o leyeras una llamada a una tool.

### Qué es un mensaje

Tres partes:

- **Rol** — quién habla: user, assistant, tool
- **Bloques de contenido** — la carga útil real
- **Metadatos** — información adicional de la respuesta del provider

### Bloques de contenido

Esta es la parte que a la mayoría se le escapa. Un mensaje no contiene una cadena. Contiene una **lista ordenada de bloques de contenido**, cada uno implementando una interfaz `ContentBlock`. NeuronAI ofrece tipos de bloque para texto, razonamiento, imagen, archivo, audio y vídeo, y proyecta cada uno automáticamente al formato correcto de cada provider.
...
`SourceType` admite `BASE64`, `URL` e `ID`. Ese tercero importa para el coste: muchos providers te permiten subir un archivo una vez a su plataforma y luego referenciarlo por ID, lo que evita volver a subir la carga en cada iteración del bucle del agente. Dada la aritmética de tokens de la Sección 1.4, eso supone un ahorro sustancial en cualquier ejecución de varios pasos que implique un documento. El Capítulo 8 lo cubre como es debido.
...
- La capa de mensajes unificada es la razón por la que el cambio de provider sobrevive al contacto con imágenes, archivos y llamadas a tools.
...
La segunda llamada envió esto al provider:
...
El modelo no recordó nada. **Tu proceso reenvió la transcripción.** No hay sesión del lado del provider, ni registro de usuario, ni nada persistido entre peticiones. El modelo lee la conversación entera de nuevo, cada vez, y responde como si recordara.
...
**2. La memoria está acotada por la context window.**
...
- La memoria cuesta tokens en cada turno y está limitada por la context window.
...
Un array. Vive solo durante el proceso PHP actual. Correcto para: scripts de una sola ejecución, endpoints de API sin estado donde el historial lo llevas tú, y tests.

Recuerda que en una petición web normal PHP muere al terminar la respuesta. Historial en memoria en un contexto web significa **ninguna memoria entre peticiones**, lo cual es una sorpresa genuinamente frecuente para desarrolladores acostumbrados a runtimes de larga vida.
...
|---|---|---|
| InMemory | Scripts, endpoints sin estado, tests | Necesitas persistencia |
...
La transcripción creció por encima del límite de contexto del modelo. El provider rechaza la petición: no la trunca por ti, devuelve un error.

El `ChatHistory` de NeuronAI lo previene recortando automáticamente. Sigue el uso de tokens a partir de las respuestas del provider y, cuando la transcripción se acerca al límite configurado, elimina mensajes del principio.

### La regla del 5–10 %

De la documentación, y vale la pena memorizarla porque es precisa y fácil de equivocar:

**Configura la context window entre un 5 y un 10 % por debajo del límite real del modelo.**
...
Este es el error contra el que conviene advertir explícitamente. Si tu provider es configurable (Sección 3.6), tu límite de contexto también lo es. Un valor escrito a fuego para un modelo de 200K se vuelve incorrecto en el momento en que alguien pone `NEURON_PROVIDER=ollama` y obtiene un modelo local de 32K.
...
**Reformula las constantes en el system prompt.** El system prompt se reenvía en cada turno y no está sujeto a recorte. Todo lo que deba sobrevivir pertenece ahí, no a la transcripción.
...
- Deduce el valor del provider, nunca lo escribas a fuego para todo el proyecto.
- El recorte descarta los mensajes más antiguos: las restricciones duraderas pertenecen al system prompt.
...
La conversación actual. Acotada por la context window, recortada automáticamente, delimitada a un hilo. Responde: «¿qué acabamos de decir?».

**2. Memoria a largo plazo — un almacén externo de hechos.**
Hechos duraderos sobre un usuario o entidad que persisten entre conversaciones. No está acotada por la context window porque no está en la transcripción: el agente la consulta bajo demanda, a través de una tool. Responde: «¿qué sé de esta persona?».
...
Fíjate bien en que esto es un **toolkit**, no un componente de historial. Esa es la afirmación arquitectónica: la memoria a largo plazo es algo que el agente *elige consultar*, mediante una llamada a una tool, no algo inyectado automáticamente en cada petición. El modelo decide cuándo un hecho merece consultarse o guardarse.
...
| «No reveles nunca los precios internos» | System prompt |
| «¿Cuántos pedidos ha hecho este cliente?» | Tool de base de datos |

Esa última fila merece énfasis, porque es el error que la gente comete más a menudo. El número de pedidos es un **hecho de tu base de datos**. No es memoria y no es RAG. Pregúntalo con SQL, a través de una tool. Recurrir a un vector store para responder una pregunta contable es un olor de diseño, y produce respuestas aproximadamente correctas, que para un recuento es lo mismo que incorrectas.
...
- La memoria a largo plazo es un **toolkit**: se consulta deliberadamente, no se inyecta de forma automática.
- Los hechos contables vienen de la base de datos, nunca de un vector store.
...
- Un error del provider se imprime en `STDERR` y te devuelve al prompt en lugar de matar el bucle.
