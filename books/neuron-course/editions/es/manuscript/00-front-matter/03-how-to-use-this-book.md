...
**Primero la teoría.** Qué está ocurriendo realmente por debajo: el bucle del agente, los tokens y el coste, los embeddings, el modelo guiado por eventos. Sin código. Estas secciones existen para que, cuando un framework haga algo por ti, sepas exactamente qué está haciendo.
...
**Parte I — Fundamentos** (Capítulos 1–2). Nada de código. La escalera de autonomía, el bucle del agente, la anatomía de una llamada a un modelo, cuánto cuesta una context window, por qué escribir prompts no es programar, dónde se sitúa NeuronAI entre los frameworks de Python y cómo está construido NeuronAI. Dos capítulos, y son los que cambian tu forma de diseñar.

**Parte II — PHP puro y Composer** (Capítulos 3–10). Configuración y tu primer agente, mensajes y memoria, tools en profundidad, structured output con validación, streaming, multimodalidad, MCP y observabilidad con evaluación en CI. Es la parte más larga del libro y su núcleo.

**Parte III — Recuperación** (Capítulos 11–12). Por qué existe la recuperación y qué no resuelve, y luego el pipeline de NeuronAI de principio a fin: loaders, splitters, embeddings, vector stores, filtrado por metadatos, reindexación y procesadores previos y posteriores.

**Parte IV — Workflows** (Capítulos 13–16). El modelo guiado por eventos, bucles y ramas, estado tipado, streaming desde dentro de un workflow, humano en el circuito con checkpoints y reanudación, y orquestación multiagente.

**Parte V — Laravel** (Capítulos 17–23). El SDK, facades e inyección de dependencias, historial de chat en Eloquent, multi-tenancy, tools sobre tus modelos reales, recuperación sobre los datos de la aplicación, streaming con SSE y Livewire, workflows de aprobación en producción, control de costes, resiliencia, seguridad y una lista de comprobación de despliegue.
...
`qwen2.5:7b` es el modelo local recomendado porque hace tool calling y structured output con solvencia, cosa que muchos modelos pequeños no logran. Si tu máquina sufre, una cuantización menor completará igualmente todos los laboratorios, solo que con menos fiabilidad, y ver a un modelo más débil fallar al elegir tools es genuinamente instructivo.

**Usa un provider de pago solo donde la calidad sea el objetivo.** Anthropic, OpenAI y Gemini aparecen en los ejercicios que dependen de la calidad de tool calling o structured output de un modelo de frontera, y en la Parte V, donde el tema es el comportamiento en producción. Cada uno de esos ejercicios lo indica al principio.

El Capítulo 3 dedica una sección entera a cambiar de provider, porque un cambio de provider de una línea es la característica más inmediatamente valiosa de NeuronAI y la que impide que te quedes atrapado mientras aún estás aprendiendo.
...
|---|---|---|
| 1 | 3 | El esqueleto del proyecto en PHP puro y tu primer agente funcionando, intercambiable entre tres providers |
| 2 | 4 | Una CLI de chat interactiva de varios turnos con historial en disco y un comando `/reset` |
| 3 | 5 | Un agente del tiempo y calculadora: un tool propio sobre una API pública más `CalculatorToolkit` |
...
| 7 | 10 | Una suite de PHPUnit sobre un agente con un provider falso y tools falsos: sin red, totalmente determinista |
| 8 | 12 | Recuperación sobre documentación a coste cero: ingesta de Markdown, `FileVectorStore`, embeddings de Ollama |
| 9 | 12 | La misma recuperación sobre un store de producción con búsqueda híbrida |
...
| 15 | 21 | Una interfaz de chat con streaming token a token y un indicador de «usando el tool X» |
| 16 | 22 | Aprobación de reembolsos de principio a fin: el agente prepara el caso, se detiene, un responsable aprueba y el workflow se reanuda |
...
Los laboratorios son el libro. Leer un capítulo sobre tools te enseña qué es un tool; escribir uno te enseña por qué las descripciones de tools son ingeniería de prompts. Presupuesta tiempo real para ellos.
...
Si necesitas algo funcionando esta semana en lugar de entender el campo, lee el Capítulo 1 (cambiará lo que decidas construir), el Capítulo 3 (configuración y primer agente), el Capítulo 5 (tools) y el Capítulo 6 (structured output). Con eso basta para publicar una funcionalidad útil de tercer peldaño.
