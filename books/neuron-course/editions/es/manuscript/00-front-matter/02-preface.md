...
La documentación oficial también se contradice a sí misma en más de cuarenta lugares: tres firmas de constructor distintas para el mismo almacén vectorial, dos API de ejecución distintas para los flujos de trabajo en páginas contiguas, nombres de clase con erratas, un método que es `public` en un ejemplo y `protected` en el siguiente. Cada uno de ellos produce un error para quien copia la página.
