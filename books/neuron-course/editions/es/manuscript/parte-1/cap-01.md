...
El único problema de ingeniería nuevo es la gestión de la memoria: qué conservas, qué descartas, qué pasa cuando el historial supera la ventana de contexto del modelo.

**Peldaño 3 — El flujo de trabajo de IA.**
...
Le das al modelo un objetivo y un conjunto de capacidades (herramientas). Tú no escribes la secuencia. El modelo elige la herramienta A, ve el resultado, decide que ahora necesita la herramienta C, ve ese resultado, decide que ya tiene bastante y escribe la respuesta final. El flujo de control es emergente, no escrito.

```
Objetivo → modelo → "llamar a la herramienta A" → ejecutar → resultado → modelo
         → "llamar a la herramienta C" → ejecutar → resultado → modelo → respuesta final
```

Ese bucle es toda la idea. Todo lo demás en este libro —memoria, RAG, flujos de trabajo, aprobación humana, observabilidad— existe para que ese bucle sea sobrevivible en producción.
...
El equipo B no puede predecir el coste por petición —el agente podría llamar a seis herramientas o a una—, no puede testear el flujo de forma determinista y, cuando se comporta mal, necesita un visor de trazas para averiguar por qué el modelo eligió la herramienta equivocada en el paso cuatro.

El equipo B tiene más poder. También tiene un problema operativo mucho más duro. Elegir el peldaño 4 es un compromiso con la observabilidad, las salvaguardas y la evaluación. Elígelo deliberadamente.
...
- La línea divisoria entre flujo de trabajo y agente es **quién escribe el flujo de control**.
...
Antes de que NeuronAI ejecute por ti el bucle de llamada a herramientas, vale la pena entenderlo mecánicamente, porque saber exactamente qué se está automatizando es la diferencia entre configurar un framework y confiar en que funcione.

### La verdad incómoda sobre la llamada a herramientas
...
El modelo nunca toca tu sistema. **Tu código es siempre quien ejecuta.** Vale la pena decirlo bien alto, porque además es el modelo de seguridad: un agente solo puede hacer aquello para lo que le diste una herramienta.
...
El modelo decide cuándo parar. Si sigue decidiendo llamar a herramientas, el bucle sigue. Por eso todo framework serio tiene una guarda de número máximo de ejecuciones, y por eso NeuronAI lanza una excepción cuando se alcanza el límite. La Sección 5.9 lo cubre.

**2. Cada iteración reenvía la conversación entera.**
El modelo no tiene estado. La quinta iteración envía el prompt de sistema, el mensaje del usuario y las cuatro llamadas a herramientas anteriores con sus resultados. El uso de tokens crece de forma *cuadrática* con la longitud del bucle. Un agente de diez pasos no es diez veces más caro que uno de un paso: es considerablemente peor que eso.

**3. Los resultados de las herramientas son solo texto.**
Lo que devuelva tu función PHP se convierte en cadena y se le entrega al modelo como contexto. Devuelve un blob JSON de 4 MB y habrás quemado tu ventana de contexto en una sola llamada. Diseñar la salida de una herramienta es diseñar prompts.
...
### Ejemplo resuelto: cómo es una ejecución con dos herramientas

El usuario pregunta: *«¿Cuál es la temperatura media actual entre Turín y Milán?»*

| Paso | Quién actúa | Qué ocurre |
|---|---|---|
| 1 | Tu código | Envía el prompt de sistema + la pregunta + 3 definiciones de herramientas |
| 2 | Modelo | Devuelve dos llamadas a herramientas: `get_weather(45.07, 7.69)`, `get_weather(45.46, 9.19)` |
...
Tres idas y vueltas al LLM. Tres ejecuciones de herramientas. Una pregunta del usuario. Interioriza esta tabla: es la mejor respuesta que existe a «¿por qué mi funcionalidad de IA es lenta y cara?».
...
- Los nombres y las descripciones de las herramientas son la única interfaz que el modelo tiene con tu sistema.

## 1.3 Anatomía de una llamada al modelo

Cada componente que entra en una sola petición se corresponde con una perilla que más adelante girarás en `SystemPrompt`, `ChatHistory` o en la definición de una herramienta. Saber cuál es cuál ahorra muchísimas conjeturas.

### Qué viaja realmente por el cable

Quita el SDK y toda petición a un proveedor son las mismas cuatro cosas:
...
- `assistant` — lo que dijo el modelo antes, incluidas las peticiones de llamada a herramientas.
- `tool` — los resultados que tu código devolvió.

El orden es la conversación. No hay sesión del lado del proveedor. Este punto conviene repetirlo hasta el aburrimiento, porque casi toda la confusión sobre la «memoria» se disuelve en cuanto aceptas que el modelo no tiene ninguna.
...
Los desarrolladores junior tratan el prompt de sistema como un saludo. Los senior lo tratan como la especificación de todo el sistema. Define:
...
Una disciplina útil: escribe el prompt de sistema como si estuvieras incorporando a un colaborador externo que es competente, rápido, no tiene contexto sobre tu empresa, no hará preguntas aclaratorias salvo que se lo digas y olvida todo entre tarea y tarea. Porque eso es exactamente lo que tienes.
...
### La ventana de contexto es un techo rígido

Todo modelo tiene un máximo: prompt de sistema + historial completo + esquemas de herramientas + la respuesta, todo junto. Si lo superas, la petición se rechaza de plano; no se trunca, se rechaza.
...
NeuronAI lo gestiona con el recorte automático del componente `ChatHistory`, y la documentación da una indicación concreta que vale la pena memorizar: **configura tu ventana de contexto entre un 5 y un 10 % por debajo del límite real del modelo.** El recortador busca un punto de corte que pierda el mínimo contexto posible, y necesita margen para encontrar uno bueno. Un modelo de 200K debería configurarse a 180–190K. Lo implementamos en la Sección 4.4.

### La aritmética de costes que cambia diseños

Los proveedores cobran los tokens de entrada y de salida por separado, y la salida suele ser varias veces más cara. Toma una tarifa plausible de gama media de 3 $ por millón de tokens de entrada y 15 $ por millón de salida.
...
Una llamada al modelo tarda de 1 a 4 segundos. Cinco iteraciones, más el tiempo de ejecución de las herramientas, y estás en 10–20 segundos de reloj. Ningún usuario espera 20 segundos ante una pantalla en blanco. Esto no es un argumento accesorio a favor de la transmisión (Capítulo 7): es la razón por la que la transmisión existe.

### Las tres palancas

Cuando la aritmética sale mal, tienes exactamente tres movimientos:

1. **Menos iteraciones** — prompts más afilados, mejor diseño de herramientas, menos herramientas.
2. **Contexto más pequeño** — recorta el historial con agresividad, devuelve resultados de herramientas compactos, resume en lugar de acumular.
3. **Modelo más barato por paso** — un modelo pequeño para enrutar y clasificar, uno grande solo para la síntesis final. La interfaz de proveedores de NeuronAI lo vuelve trivial, y es uno de los argumentos más fuertes a favor del framework.
...
Ejecuta el mismo código dos veces y obtén el mismo resultado. Esa suposición sostiene las pruebas unitarias, la depuración, la revisión de código y la CI. Un LLM la viola. Mismo prompt, mismo modelo, mismos parámetros: salida distinta.

Ni siquiera a temperatura cero obtienes determinismo real: la no asociatividad de los números en coma flotante entre lotes de GPU, actualizaciones del modelo del lado del proveedor detrás de un alias estable, enrutado dependiente de la carga. Trata «temperatura 0 significa determinista» como falso en producción.

### Qué se rompe

**Las pruebas unitarias tal como los conoces.** `assertEquals($expected, $agent->chat($input))` no pasará dos veces.
...
**El versionado semántico de tu propio sistema.** Tu código no cambió y tu comportamiento sí. El proveedor actualizó un modelo detrás de un alias.

### Qué sobrevive, y qué sustituye al resto

**Testear contratos en lugar de salidas.** No hagas asertos sobre el texto. Hazlos sobre la forma: ¿devolvió JSON válido conforme al esquema?, ¿llamó a la herramienta esperada?, ¿está presente el campo obligatorio? La salida estructurada (Capítulo 6) existe en gran medida para hacer esto posible.
...
**Fijar versiones en lugar de confiar en alias.** Usa versiones de modelo explícitas en producción —un identificador de modelo fechado y plenamente cualificado en lugar de un alias móvil— para que una actualización del proveedor sea un despliegue que eliges y no un incidente que descubres.

### El cambio mental

Deja de pensar «función». Empieza a pensar «un colega junior competente pero inconsistente». No harías pruebas unitarias a un colega. Le darías instrucciones claras, limitarías sus permisos, revisarías su trabajo y seguirías su tasa de error. Todos los patrones arquitectónicos de este libro son una de esas cuatro cosas.

### Puntos clave

- Da por hecho el no determinismo incluso a temperatura 0.
- Testea contratos y formas, no cadenas.
- Componentes falsos para la CI; evaluaciones para la calidad; trazas para depurar.
...
El apaño estándar es un microservicio en Python junto a la aplicación PHP. Eso significa un segundo entorno de ejecución, un segundo pipeline de despliegue, un segundo árbol de dependencias, un límite de API que ahora tienes que diseñar, autenticar y versionar y, algo crítico, el agente vive al lado equivocado de ese límite respecto de las reglas de negocio. Cada llamada a una herramienta se convierte en un salto de red hacia una aplicación que ya sabía la respuesta.

### Dónde encaja NeuronAI

NeuronAI es la implementación nativa en PHP de la misma arquitectura. Concretamente ofrece:

- Un bucle de agente con llamada a herramientas, sobre muchos proveedores detrás de una sola interfaz
- Un motor `Workflow` guiado por eventos con estado, bucles, puntos de control e interrupción: la pieza con forma de LangGraph
- Un pipeline de RAG: cargadores, incrustaciones, almacenes vectoriales, procesadores previos y posteriores
- Soporte de cliente MCP
- Transmisión con adaptadores de protocolos de interfaz
...
No tienes por qué usar un framework. Puedes llamar a la API de un proveedor con Guzzle y escribir tú mismo el bucle de herramientas: quizá 150 líneas. Lo que entonces te pertenece es: la abstracción multi-provider, los reintentos, el parseo de la transmisión, la generación de esquemas, el recorte del historial, los puntos de control y un formato de trazas. Ese es el alcance honesto de lo que te ahorra un framework. Juzga a NeuronAI con esa lista.
...
- Los frameworks te compran abstracción de proveedores, transmisión, esquemas, gestión del historial, puntos de control y trazas.
...
Esta es toda la cuestión. Si puedes dibujar el diagrama de flujo, escribe el diagrama de flujo. Un flujo de trabajo que siempre hace las mismas tres cosas es más barato, más rápido, testeable y depurable.

**P2. ¿Necesita leer o escribir en sistemas de registro?**
No → no hacen falta herramientas. Sí → herramientas, e inmediatamente: qué permisos, qué traza de auditoría, qué acciones son irreversibles.

**P3. ¿Alguna acción es irreversible o cara?**
Sí → el humano en el circuito es obligatorio, no un objetivo aspiracional. Reembolsos, correos a clientes, borrados, pagos. Diseña la puerta de aprobación al mismo tiempo que la herramienta, nunca después.

**P4. ¿La corrección depende de conocimiento privado?**
Sí → RAG o herramientas de base de datos. Fíjate en que son respuestas distintas: RAG para prosa no estructurada, herramientas de base de datos para hechos estructurados. Usar RAG para responder «cuántos pedidos hizo este cliente» es un error de diseño: eso es una consulta SQL, y el modelo debería llamarla como herramienta.
...
→ **Peldaño 4, con herramientas, RAG, puertas de aprobación y trazado completo.** Este es el Proyecto final B del Capítulo 25, y se gana cada pieza de la maquinaria de este libro.
...
- Las acciones irreversibles exigen una puerta de aprobación diseñada en el mismo momento que la herramienta.
- RAG para prosa, herramientas de base de datos para hechos: no los confundas.
